...
\pagebreak
\section{Training in the Function Space}

The goal of this section is to introduce and investigate the neural tangent kernel, which was analyzed by Arthur Jacot, Franck Gabriel and Cl\'{e}ment Hongler in \cite{NTK}. \\
A \\
A \\
A \\
A \\
...
\[ \mu '_\phi [ f_0 ] \coloneq \lim_{\epsilon \to 0} \frac{1}{\epsilon} \Big ( \mu[f_0 + \epsilon \phi] - \mu[f_0] \Big ) = \frac{\partial}{\partial \epsilon} \Big [ \mu[f_0+\epsilon \phi] \Big ]_{\epsilon=0} \]
exists. If the limit exists for every $\phi \in \F$ we define the functional derivative of $\mu$ at $f_0$ as
\[ \partial_{f} \mu |_{f_0} : \F \to \R : \phi \mapsto \mu '_\phi [ f_0 ] \]
and call $\mu$ differentiable at $f_0$.
\end{definition}

This is analog to the notion of directional derivatives in $\R^{n_0}$. Likewise we can motivate the functional gradient in $\F$ based on the properties of the classical gradient in $\R^{n_0}$. To clarify this, we let $f \in \F$ and $x,v \in \R^{n_0}$, such that $\| v \|_2 = 1$ and consider the directional derivative of $f$ in $x$ along the direction $v$, which is given by
\[ f'_v(x) \coloneq \lim_{\epsilon \to 0} \frac{1}{\epsilon} \Big ( f(x + \epsilon v) - f(x) \Big ). \]
The gradient $\nabla f(x)$ can be thought of as the best linear approximation of $f$ in $x$.
\[ f(x + \epsilon v ) - f(x) = \epsilon \big \langle \nabla f(x), v \big \rangle + \O (\epsilon) \]
we derive 
\[ f'_v(x) = \big \langle \nabla f(x), v \big \rangle. \]
Roughly speaking we could translate this concept and define the functional gradient of a functional $\mu: \F \to \R$ at $f_0 \in \F$ as an element $d \in \F$, such that
\[ \forall \phi \in \F : \mu '_\phi [ f_0 ] = \langle d, \phi \rangle_\F, \]
for a reasonable inner product $\langle \cdot, \cdot \rangle_\F$. It will turn out, that this is completely possible. We will start with the definition of an inner product and show the existence of the functional gradient afterwards. The following lemma is based on \cite{NTK}.
...
Note that $\langle \cdot, \cdot \rangle_{\D_x}$ is not strictly positive definite. For example, in the case where $\D_x$ is a discrete distribution, any $f \in \F$ mapping the support of $\D_x$ to zero, satisfies $\langle f, f \rangle_{\D_x} = 0$. However, for our setting only the datapoints with a positive probability measure in terms of $\D_x$ are of interest. That is why we can define an equivalence relation on $\F$, such that
\[ \forall f,g \in \F \ : \ f \sim g \ \Leftrightarrow \ \langle f, \cdot \rangle_{\D_x} = \langle g, \cdot \rangle_{\D_x}. \]
Thus $\langle \cdot, \cdot \rangle_{\D_x}$ restricted to $\F / \sim$ is strictly positive definite, making $\langle \cdot, \cdot \rangle_{\D_x}$ an inner product. Based on the following lemma, $\F / \sim$ is complete, such that $\big ( F / \sim, \langle \cdot, \cdot \rangle_{\D_x} \big )$ is a Hilbert space.
...
represents a limit function of the Cauchy sequence. This is due to the limit being measured in terms of the seminorm $\| \cdot \|_{\D_x}$. \\

Based on the notion of $\langle \cdot, \cdot \rangle_{\D_x}$, the functional gradient of some differentiable $\mu : \F \to \R$ at $f_0 \in \F$ could be thought as an element $\nabla \mu |_{f_0} \in \F$, that satisfies
\[ \forall \phi \in \F : \mu'_{\phi}[f_0] = \big \langle \nabla \mu |_{f_0}, \phi \big \rangle_{\D_x}. \]
Note that, due to the definition of $\langle \cdot, \cdot \rangle_{\D_x}$, the functional gradient is not unique and depends only on the support of $\D_x$. However we will discuss and address this problem later on, but first let us prove the existence of functional gradients. \\

We will see, that the functional gradient exists, if $\mu'_\phi[f_0]$ is linear and continuous. For this we denote the class of linear and continuous functionals on $\F$, namely the dual space of $\F$.
...
such that we can conclude the existence based on $\mu'_\phi[f_0] \in \F^*$. For the construction of $J$ we have to translate some well known results from functional analysis to our setting. \\
...
Note that since $\| \cdot \|_{\D_x}$ is only a seminorm and not a norm, the limit of $(a_n)_{n=1}^\infty$ is not unique. Essentially there can exist multiple orthogonal projections from $f$ onto the subset $\A$. \\
...
\[ L : \F / \sim \to \R : [\phi] \mapsto \mu_\phi'[f_0]. \]
The linearity follows from the definition of $\partial_f\mu |_{f_0}$. To prove this, we let $\phi_1, \phi_2 \in \F$ and derive
\[ \begin{split} 
...
&= \partial_f \mu |_{f_0} [\phi_1] + \partial_f \mu |_{f_0} [\phi_2] \end{split}. \]
...
Using that $\D_x$ is the empirical distribution on a finite subset of $\R^{n_0}$, we have, that $\F / \sim$ is a finite-dimensional normed space. Thus there exists a basis ... and we conclude, that the operator $L$ is bounded. Using that, in combination with the linearity of $L$, we obtain that 
\[ \big \| L(x+h) - L(x) \big \| = \big \| L(h) \big \| \leq M \big \| h \big \|. \]
Taking the limit $h \to 0$ yields the continuity of $L$ at $x$. Thus $L$ is a bounded and continuous operator. This implies, that $\partial_f\mu |_{f_0}$ is indeed linear and continuous.
\end{proof}

Lemma \ref{lem:riesz} is a weakening of the Riesz representation theorem, stating that any Hilbert space $\H$ is isometric and isomorphic to its dual space $\H^*$ via the map $J:\H \to \H^* : x \mapsto \langle x , \cdot \rangle_\H$. However, since $\langle \cdot , \cdot \rangle_{\D_x}$ is only positive semidefinite and not positive definite we lose the property of $J$ being isometric and therefore injective. This is a severe problem for our approach of minimizing a functional via gradient descent, because the functional gradient is not uniquely determined. Namely there could exist several descent directions that could be chosen for the iterative updates, but we have no criterium to choose one of them. To clarify this, based on the definition of $\langle \cdot, \cdot \rangle_{\D_x}$, the functional gradient $\nabla \mu |_{f_0}: \R^{n_0} \to \R^{n_l}$ is only unique on the datapoints $x \sim \D_x$. However, since $\nabla \mu |_{f_0}$ is a function on $\R^{n_0}$, in the case where $\text{supp}(\D_x) \subsetneq \R^{n_0}$ there is no unique descent direction for points $x \in \R^{n_0}$ with $x \notin \text{supp}(\D_x)$. \\

Kernel gradient descent is an algorithm, that addresses this problem by minimizing the cost $C$ in a special subspace of $\F$, namely a reproducing kernel Hilbert space, in which the functional gradient is uniquely determined. This idea will be explained in detail in section \ref{sec:KGD}, but first we need some background knowledge on kernels and reproducing kernel Hilbert spaces.

\subsection{Reproducing Kernel Hilbert Space}

To explain the approach of minimizing the cost functional in a reproducing kernel Hilbert space, we start with the following results from \cite{NTK}.
...
Further on, we will always work with multi-dimensional kernels $K: \R^{n_0} \times \R^{n_0} \to \R^{n_l \times n_l}$ and for reasons of simplicity just call them kernels. At first we observe, that kernels induce symmetric bilinear forms based on the marginal distribution $\D_x$.
...
To construct a Hilbert space, it will be necessary, that $\langle \cdot, \cdot \rangle_K$ is positive definite.

\begin{definition} \label{def:definite}
We say that a kernel $K$ is positive definite with respect to $\D_x$, if the corresponding bilinear form $\langle \cdot, \cdot \rangle_K$ is positive definite, that is for any $f \in \F$ we have
\[ \langle f,f \rangle_K \geq 0 \quad \wedge \quad \langle f,f\rangle_K = 0 \Leftrightarrow f \equiv 0. \]
\end{definition}

Note that, taking lemma \ref{lem:form} and definition \ref{def:definite} together, positive definite kernels induce an inner product $\langle \cdot, \cdot \rangle_K$. According to Aronszajn in \cite{RKHS} we denote the following theorem.

%To simplify the following notation we introduce for $m \in \N$ the symbol
%\[ \N_m \coloneq \{ n \in \N \mid n \leq m \}. \]

\begin{theorem}  \label{thm:aronszajn}
Let $K$ be a kernel that is positive definite with respect to $\D_x$ and define
\[ \H_0 \coloneq \text{span} \Big \{ K_xy : \R^{n_0} \to \R^{n_l} : z \mapsto K(z,x)y \mid x \in \R^{n_0}, y \in \R^{n_l} \Big \}, \]
then the closure $\H_K \coloneq \overline{H_0}$ is a Hilbert space with inner product $\langle \cdot, \cdot \rangle_{K}$.
\end{theorem}
...
This is almost the definition of a reproducing kernel Hilbert space. The following idea is from \cite{Reproducing}.

\begin{lemma} \label{lem:RKHS}
Let $K$ be a kernel that is positive definite with respect to $\D_x$. In the case where $\D_x > 0$ for every $x \in \R^{n_0}$ we can use the notation 
\[ f = \sum_{i} K_{x_i}y_i \quad \text{and} \quad g = \sum_{i} K_{\hat{x}_i}\hat{y}_i \]
for $f,g \in \H_K$ and define an inner product on $\H_k$ as
\[ \langle \cdot, \cdot \rangle_{\H_K} : \H_K \times \H_K \to \R : (f,g) \mapsto \sum_{i,j} y_i \tr K(x_i, \hat{x}_j) \hat{y}_j. \]
We call $ \big (\H_K, \langle \cdot, \cdot \rangle_{\H_K} \big )$ the vector-valued reproducing kernel Hilbert space associated with $K$.
\end{lemma}

\begin{proof}
We need to show that $\langle \cdot, \cdot \rangle_{\H_K}$ is an inner product on $\H_K$.
\end{proof}
...
\begin{proof}
Let $f = \sum_{i}K_{x_i}y_i \in \H_K$ and $x \in \R^{n_0}, y \in \R^{n_l}$ be arbitrary. We observe that
\[ \langle f(x), y \rangle = \sum_{i} \big ( K(x,x_i)y_i \big ) \tr y = \sum_{i,j} y_i \tr K(x_i,x)y = \langle f, K_xy \rangle_{\H_K}. \qedhere \]
\end{proof}

In our setting the requirement $\D_x > 0$ for every $x \in \R^{n_0}$ may not be given. For example consider the case of a discrete distribution. Then, by the proof of lemma \ref{lem:RKHS}, we observe that the bilinear form $\langle \cdot, \cdot \rangle_{\H_K}$ is not necessarily positive definite, such that $\big ( \H_K, \langle \cdot, \cdot \rangle_{\H_K} \big )$ is no longer a Hilbert space. However, when restricting $\H_K$ to functions in terms of data points with a positive probability measure, $\langle \cdot, \cdot \rangle_{\H_K}$ is still positive definite. Since these are exactly the functions of our interest, we can still think of $\H_K$ as a vector-valued reproducing kernel Hilbert space, although the requirement $\D_x > 0$ for every $x \in \R^{n_0}$ may not be given.

\subsection{Kernel Gradient Descent} \label{sec:KGD}

This subsection is dedicated to minimizing a cost functional $C: \F \to \R$ via kernel gradient descent, a generalization of gradient descent from the parameter space $\R^n$ to the function space $\F$. As explained in the previous section, we have the problem, that the functional gradients are not unique in contrast to the parameter gradients. The idea of kernel gradient descent consist of minimizing the cost $C$ in a special subspace of $\F$, namely a reproducing kernel Hilbert space, in which the functional gradient is uniquely determined. The results of this subsection are taken from \cite{NTK}.
...
\[ K_i : \R^{n_0} \times \R^{n_0} \to \R^{n_l} : (x,x') \mapsto K(x,x')_{\cdot,i}, \]
where $K(x,x')_{\cdot,i}$ denotes the $i$-th column of the $n_l \times n_l$ matrix $K(x,x')$.
\end{definition}

Fixing one argument of $K_i$ results in a function in $\F$. This enables the following definition.
...
\[ f_{\mu} : \R^{n_0} \to \R^{n_l} : x \mapsto  \begin{bmatrix} \mu \big [K_1(\cdot,x) \big ] \\ \vdots \\ \mu \big [ K_{n_l}(\cdot,x) \big ] \end{bmatrix}. \]
\end{definition}

Based on the definition of $\Phi_K$ we can define the kernel gradient of a functional.

\begin{definition}
Let $f_0 \in \F$, $\mu : \F \to \R$ be a differentiable functional and $K$ be a kernel. The kernel gradient of $\mu$ at $f_0$ with respect to $K$ is defined as
\[ \nabla_K\mu|_{f_0}: \R^{n_0} \to \R^{n_l} : x \mapsto \Phi_K \Big ( \partial_f\mu|_{f_0} \Big )(x). \]
\end{definition}

Note that the kernel gradient is well-defined based on the restriction, that $\D_x$ is given by the empirical distribution on a finite dataset. This was proven in lemma \cite{lem:funcDerivative}, which states, that  $\partial_f \mu |_{f_0} \in \F^*$ for every $f_0 \in \F$. By Lemma \ref{lem:riesz} there exists a representer $d|_{f_0} \in \F$, such that the functional derivative of $C$ can be written as
\[ \partial_{f} C |_{f_0} = \big \langle d|_{f_0}, \cdot \big \rangle_{\D_x}. \]

Based on this observation, we can interpret the kernel gradient of $C$ with respect to a positive definite kernel $K$ as an element of the vector-valued reproducing kernel Hilbert space $\H_K$. To clarify this, we denote the following result.
...
\[ \Phi_K \big ( \mu \big )(x) = \Phi_K \Big ( \langle d, \cdot \rangle_{\D_x} \Big )(x) = \begin{bmatrix} \mu \big [K_1(\cdot,x) \big ] \\ \vdots \\ \mu \big [ K_{n_l}(\cdot,x) \big ] \end{bmatrix} = \begin{bmatrix} \big \langle d, K_1(\cdot,x) \big \rangle_{\D_x} \\ \vdots \\ \big \langle d, K_{n_l}(\cdot,x) \big \rangle_{\D_x} \end{bmatrix}. \]
Recalling the definition of $\langle \cdot , \cdot \rangle_{\D_x}$, it holds that
\[ \big \langle d, K_i(\cdot,x) \big \rangle_{\D_x} = \E_{x' \sim \D_x} \Big [ d(x')\tr K_i(x',x) \Big ] \]
for $i = 1, \dots, n_l$. Thus we derive
\[ \Phi_K \big ( \mu \big )(x) = \begin{bmatrix} \big \langle d, K_1(\cdot,x) \big \rangle_{\D_x} \\ \vdots \\ \big \langle d, K_{n_l}(\cdot,x) \big \rangle_{\D_x} \end{bmatrix} = \E_{x' \sim \D_x} \begin{bmatrix} d(x')\tr K_{1}(x',x) \\ \vdots \\ d(x')\tr K_{n_l}(x',x) \end{bmatrix} = \E_{x' \sim \D_x} \Big [ d(x')\tr  K(x',x) \Big ]\tr . \]
...
Hence in the case where the kernel $K$ is positive definite, $\Phi_K$ from definition \ref{def:phi} maps a dual element $\mu \in \F^*$ to the vector-valued reproducing kernel Hilbert space $\H_K$, namely
\[ \Phi_K: \F^* \to \H_K : \mu \mapsto \frac{1}{m} \sum_{i=1}^{m} K_{x_i}y_i, \]
where $y_i \coloneq d(x_i)$. For the kernel gradient of $C$ at $f_0$ this translates to
\[ \nabla_KC|_{f_0}(x) = \Phi_K \Big ( \big \langle d|_{f_0} , \cdot \big \rangle_{\D_x} \Big )(x) = \frac{1}{m} \sum_{i=1}^{m} K(x,x_i)d|_{f_0}(x_i). \]

Thus, in contrast to the functional gradient, the kernel gradient generalizes to values outside the dataset thanks to the kernel $K$. Based on the definition of kernel gradient, we can finally introduce the kernel gradient descent algorithm.
...
The kernel gradient enables us to establish a relationship between training in the function space and training in the parameter space. Before we explain this in detail, we prove that kernel gradient descent actually minimizes the cost $C$, although our descent direction is taken from a subspace $\H_K \subset \F$.
...
\begin{definition}
Let $K$ be a kernel. A time dependent function 
\[ f : [0, \infty) \to \F : t \mapsto f(t) \]
is said to follow the kernel gradient descent on $C$ with respect to K if it satisfies
\[ \partial_tf(t) = - \nabla_KC|_{f(t)}. \]
\end{definition}

Roughly speaking we can think of such a function as the continuous extension to the sequence produced by kernel gradient descent on $C$, when using an infinitely small step size. Based on this we can denote the following result, to prove  convergence of kernel gradient descent.

\begin{lemma}
Let $K$ be a positive definite kernel with respect to $\D_x$ and $f: [0, \infty) \to \F$ be a time dependent function, that follows the kernel gradient descent on $C$ with respect to $K$, then the evolution of $C \circ f$ during kernel gradient descent is expressed by
...
\begin{proof}
Using the notation $C|_{f(t)} = C [f(t)]$, we derive
...
\textbf{To Do - Section 3.4} \\
Norm in kernel gradient descent \\
Why is $\partial_{f} C |_{f_0}$ linear and continuous? \\
Why does the first equality in proof of lemma 3.21 holds? \\

\textbf{To Do - Section 3.5} \\
Why does the first equality in proof of lemma 3.25 holds? \\
Explain equivalence between the gradient methods. \\
...
The goal of this section is to introduce and investigate the neural tangent kernel, which was analyzed by Arthur Jacot, Franck Gabriel and Cl\'{e}ment Holger in \cite{NTK}. We start by motivating and giving the definition of functional derivatives and functional gradient descent. Then, the repetition of some background knowledge on dual spaces and reproducing kernel Hilbert spaces, enables us to introduce the kernel gradient descent algorithm. Having this concept in mind, we prove that training in the parameter space is linked to kernel gradient descent in the function space with respect to a special kernel, the neural tangent kernel.
...
\textcolor{white}{$\Big |$}2: Initialize $k \leftarrow 0$. \\
\textcolor{white}{$\Big |$}3: \textbf{while} $\big \| \nabla \mu |_{f_k} \big \|_{\F} > \epsilon$ \textbf{do}: \\
\textcolor{white}{$\Big |$}4: \quad Choose a step size $\alpha_k > 0$. \\
...
Thus, the choice of a convex loss $\ell$, which yields a convex cost functional $C$, enables us to guarantee convergence of functional gradient descent to a global minimum. As said before, this is great for theoretical considerations. In practice however, we are still in need of the explicit parameters $w \in \R^n$, that describe the objective function. In general, one can parametrize any function in a number of ways, such that each parametrization gives rise to different parameter updates in gradient descent. That is why there is no direct connection between functional gradient descent and gradient descent in the parameter space. \\

To still derive conclusions for the parameter training based on the properties of training in the function space, we need to establish a relationship between the two minimization approaches. It turns out, that this can be achieved by minimizing $C$ in a special subspace of $\F$, namely a reproducing kernel Hilbert space. This modified version of functional gradient descent is called kernel gradient descent and will be studied in section \ref{sec:KGD} in detail. But first we need some background knowledge from functional analysis.
...
At this point it is unclear how all of the elements in $\F^*$ arise. As mentioned, they can be constructed by elements of the function space $\F$, for which we need to equip  $\F$ with a seminorm. This procedure is based on the following result from \cite{NTK}.
...
Just as norms induce metrics, seminorms induce pseudometrics, so that we can define a distance between two elements of $\F$ in terms of the bilinear form $\langle \cdot, \cdot \rangle_{\D_x}$.
...
Thus for every $x \sim \D_x$ it holds that $f(x) = 0$. Therefore we conclude THIS IS NOT TRUE
...
This concludes the necessary knowledge on kernels.
...
\bibitem{RKHS} Nachman Aronszajn. \textit{Theory of Reproducing Kernels}. Transactions of the American Mathematical Society, 686:337-404, 1950.

\bibitem{Reproducing} H\`{a} Quang Minh, Vikas Sindhwani. \textit{Vector-valued Manifold regularization}. ICML'11: Proceedings of the 28th International Conference on International Conference on Machine Learning, pages 57-64, 2011.
