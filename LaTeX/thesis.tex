...
\[ \nabla E(w_k,I_k) := \frac{1}{b} \sum_{i \in I_k}^{}  \ell \big ( f(x_i,w_k),y_i \big) + \lambda \| w_k \|^2 \]
...
\[ \Sigma := \text{diag}(\sigma_1, \dots, \sigma_p) \in \R^{m \times n} \]
with $p=\text{min} \{ m,n \}$ and singular values $\sigma_1 \geq \sigma_2 \geq \cdots \geq \sigma_p \geq 0$, such that
\[ A = U \Sigma V^T. \]
\end{theorem}
...
\[ A^k := \sum_{i=1}^{k} u_i \sigma_i v_i^T = \tilde{U} \tilde{\Sigma} \tilde{V}^T, \]
...
This concludes the necessary background knowledge on singular value decomposition and low-rank approximations, which will be useful for the subspace extraction. \\
...
\[ \F := \Big \{ f_w : \R^d \to \R : x \mapsto f(x,w) \mid w \in \R^n \Big \}. \]
...
 \\
\textbf{Last problem:} Chain rule for functionals.
